...
function handleLetterClick(letter, buttonElement) { /* ... (implementation as before) ... */
    ...
    if (currentPath.length > 0) { 
        ...
        if (findSide(letter) === findSide(previousLetter)) { 
            showMessage(`Cannot use two letters ("${previousLetter}", "${letter}") from the same side`); 
            return; 
        } 
    }
    ...
    if (currentWord.length === 1 && lastLetterOfPreviousWord) { 
        if (findSide(letter) === findSide(lastLetterOfPreviousWord)) { 
            showMessage(`Cannot use two letters ("${lastLetterOfPreviousWord}", "${letter}") from the same side`); 
            return; 
        } 
    }
    currentWord += letter; 
...
function validateWordRules(word) { 
    if (!word || word.length < 3) { 
        ...
        return false; } 
    if (!/^[A-Z]+$/.test(word)) { 
        showMessage("No special characters"); 
        return false; 
    } 
    ...
    for (let i = 0; i < word.length - 1; i++) { 
        ...
        if (!currentSide || !nextSide) { 
            console.error("Side validation error"); 
            showMessage("Internal error."); 
            return false; } 
        if (currentSide === nextSide) { 
            showMessage(`Cannot use two letters ("${word[i]}", "${word[i + 1]}") from the same side`); 
            return false; 
        } 
    }
...
function checkWordWithAPI(wordToCheck) { 
    ...
    return fetch(`https://api.dictionaryapi.dev/api/v2/entries/en/${wordToCheck}`) .then(response => { 
        ...
        if (error.message === 'WordNotFound') { 
            ...
            showMessage("Error checking word validity."); 
...
function submitWord() { 
    ...
    checkWordWithAPI(wordToSubmit.toLowerCase()) .then(data => { 
        console.log("API Data for valid word:", data); 
        clearMessage(); updateGameState(wordToSubmit); 
        if (isTyped) { 
            typedWordInput.value = ""; 
        }}) .catch(() => {});
}

/** Clears only the current click path/word attempt */
